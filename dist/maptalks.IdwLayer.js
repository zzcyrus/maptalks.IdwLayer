...
IdwLayer.registerRenderer('canvas', function (_maptalks$renderer$Ca) {
    ...
    _class.prototype._idwData = function _idwData(idws) {
        ...
        var data = [],
            r = this._idw._r,
            size = map.getSize(),
            bounds = new maptalks.Extent(new maptalks.Point([-r, -r]), new maptalks.Point([size.width + r, size.height + r])),
        ...
        for (i = 0, len = nCellY; i < len; i++) {
            for (j = 0, len2 = nCellX; j < len2; j++) {
                ...
                for (k = 0, len3 = idws.length; k < len3; k++) {
                    var p = map.coordinateToContainerPoint(new maptalks.Coordinate([idws[k][0], idws[k][1]]));
                    var cp = new maptalks.Point(y - cellCen, x - cellCen);
